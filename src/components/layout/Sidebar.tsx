...
export default function Sidebar() {
  ...
  const meta = PAGE_META[pathname] || { title: 'FitHub' };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span className="text-base font-bold gradient-text">Max Muscle</span>
        </div>
      </div>
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex lg:flex-col lg:w-56 lg:fixed lg:inset-y-0 bg-[#0f0f0f] border-r border-white/5 z-30">
        <NavContent />
      </aside>

      {/* Mobile top header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 h-14 bg-[#0f0f0f]/95 backdrop-blur-lg border-b border-white/5 flex items-center px-4 gap-2">
        {meta.back ? (
          <button
            onClick={() => router.push(meta.back!)}
            className="p-2 text-slate-400 hover:text-white transition-colors -ml-1 min-h-[44px] min-w-[44px] flex items-center justify-center"
            aria-label="Go back"
          >
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="font-semibold text-white text-sm truncate">{meta.title}</span>
        </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0f0f0f]/98 backdrop-blur-lg border-t border-white/8 grid grid-cols-6">
        {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
          const active = pathname === href;
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex flex-col items-center justify-center py-2 px-0.5 gap-0.5 min-h-[56px] transition-all active:scale-95',
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-64 bg-[#0f0f0f] h-full flex flex-col shadow-2xl">
            <button
              onClick={() => setMobileOpen(false)}
