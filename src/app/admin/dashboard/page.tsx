...
export default function AdminDashboardPage() {
  ...
  const STATS = [
    { label: 'Total Members', value: users.length, icon: Users, color: 'indigo' as const },
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <Link href="/admin/qr" className="flex items-center gap-1.5 px-2.5 py-2 bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 rounded-xl text-xs font-medium hover:bg-indigo-600/30 transition-all">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {STATS.map(({ label, value, icon: Icon, color }) => (
            <Card key={label} className="flex items-center gap-3 p-4">
              <div className={`p-2.5 rounded-xl flex-shrink-0 bg-${color}-500/10 text-${color}-400`}>
          ...
          <>
            <div className="mb-3">
              <input
                ...
                className="w-full max-w-sm bg-[#0f0f0f] border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-amber-500"
              />
            </div>
            ...
            <Card className="overflow-hidden p-0">
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b border-white/5">
                    <tr>
                      {['Name', 'Phone', 'Status', 'Days Left', 'Joined', 'Renewed', 'Actions'].map((h) => (
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filteredUsers.map((u) => {
                      ...
                      return (
                        <tr key={u.id} className={`hover:bg-white/2 transition-colors ${u.banned ? 'opacity-60' : ''}`}>
                    ...
                    {filteredUsers.length === 0 && (
                      <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">No members found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="md:hidden divide-y divide-white/5">
                {filteredUsers.map((u) => {
                  ...
                  return (
                    <div key={u.id} className={`p-4 ${u.banned ? 'opacity-60' : ''}`}>
          ...
          <Card className="overflow-hidden p-0">
            ...
            <div className="divide-y divide-white/5">
              ...
              {todayAttendance.length === 0 && (
                <p className="px-4 py-10 text-center text-slate-500">No check-ins today yet</p>
              )}
            </div>
          </Card>
        )}
              ...
              <form onSubmit={handleRenew} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#000000] border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:border-amber-500"
                  >
                    {DURATIONS.map((m) => (
                      <option key={m} value={m}>{m} month{m > 1 ? 's' : ''}</option>
                    ))}
                ...
                <button
                  ...
                  className="w-full h-12 bg-amber-600 hover:bg-amber-500 text-white font-semibold rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
