...
export default function SignupPage() {
  ...
  async function handleStep1(e: React.FormEvent) {
    ...
    // Move to step 2 (body metrics)
    setStep(2);
  }

  // ── Step 2: save body metrics then go to dashboard ─────────────────────
  async function handleStep2(e: React.FormEvent) {
    ...
    if (body.height_cm && (isNaN(h) || h < 50 || h > 300)) {
      setError('Enter a valid height (50–300 cm)');
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-6">
          {step === 1 ? (
            <Link href="/" className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm transition-colors min-h-[44px]">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          {step === 1 ? (
            ...
            <>
              ...
              <p className="text-sm text-slate-400 mt-1 text-center">Helps us calculate BMI & smart recommendations</p>
            </>
          )}
              ...
              <div className="grid grid-cols-1 gap-2">
                {GOALS.map((g) => (
                  <button
                    key={g.value}
                    type="button"
                    onClick={() => setBody((b) => ({ ...b, goal: g.value }))}
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                      body.goal === g.value
                        ? 'border-red-600/60 bg-red-700/10 text-white'
